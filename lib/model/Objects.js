 ...
 * @class Objects
 * @desc
 * Objects are elements in the Repository that correspond to a set of properties and a blob (binary or string) of {@link SeedData}
...
/**
 * Retrieve an Object from the repository database by it's type and ID
 * @param {string} token - API-Auth token retrieved with {@link Tokens}
 ...
 *
 * @param {?Error} err - {@link null} or an object describing an exception that occured during execution
 * @param {?(String|Object)} responseBody - the body of the response, either a POJO from the JSON response or String with the textual data
 *
 * @desc
...
/**
 * Update an object based on it's ID in the repository
 *
 * @param {string} token - API-Auth token retrieved with {@link Tokens}

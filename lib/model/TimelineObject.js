 ...
 */
function TimelineObject(serviceUrl, sourceType) {
...
TimelineObject.prototype.createTimelineObject = function(token, props, callback) {
    ...
    api(token, this.endpoint, "POST", props, this.source, function(error, responseBody) {
        if (error) {
            // Request failed.
            callback(error, null);
        } else {
 ...
 * @param {function} callback -
 * [Called after the API request was finalized]{@link TimelineObject~timelineObjectExists(callback)}
 */
TimelineObject.prototype.timelineObjectExists = function(token, id, callback) {
    log.debug("timelineObjectExists(token=" + token + "),  id=" + id +")");
...
/**
 * @callback TimelineObject~timelineObjectExists(callback)
 *
 * @param {?Error} err - {@link null} or an object describing an exception that occured during execution
...
/**
 * Update an timelineitem based on its ID in the repository
 *
 * @param {string} token - API-Auth token retrieved with {@link Tokens}
 * @param {string} id - the id of the object to update
 * @param {Object} props - new properties for the object in the repository
 * @param {function} callback -
 * [A callback function called after the API request was finalized]{@link TimelineObject~updateTimeline(callback)}
 */
TimelineObject.prototype.updateTimelineObject = function(token, id, props, callback) {
    log.debug("updateTimelineObject(token=" + token + ", id=" +
        id + ", props=" + JSON.stringify(props) + ")");

    var objectURL = this.getObjectURL(id);
    api(token, objectURL, "PUT", props, this.source, callback);
};
/**

 ...
 */
function Tokens(serviceUrl, sourceType) {
...
Tokens.prototype.connect = function(tenantName, userName, password, callback) {
    ...
    api(null, this.endpoint, "PUT", credentials, this.source, function(error, responseBody) {
        if (error) {
            // Request failed.
            callback(error, null);
        } else {
 ...
 * @param {?string} token - Authentication token returned upon successful authentication
 */
...
Tokens.prototype.getToken = function(token, callback) {
    api(null, this.endpoint + "/" + token, "GET", null, this.source, callback);
}
/**
 * @callback Tokens~getToken(token, callback)
 *
 * @param {?Error} err - {@link null} or an Error object describing the type of Exception that occurred
...
Tokens.prototype.disconnect = function(token, callback) {
    ...
    api(null, tokenURL, "DELETE", null, this.source, callback);
};
/**

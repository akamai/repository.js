...
/**
 * Authenticate with the API endpoint to get the entire token object
 * 
 * @param {string} tenantName - name of the tenant we want to access
 * @param {string} apiToken - the apiToken retrieved from mPulse UI
 * @param {function} callback - 
 * [Called after the API Auth request was finalized]{@link Tokens~getToken(callback)}
 */
Tokens.prototype.getToken = function(tenantName, apiToken, callback) {
    var credentials = {
        apiToken: apiToken
    };
    ...
    api(null, this.endpoint + "/" + apiToken, "GET", credentials, function(error, responseBody) {
        if (error) {
            // Request failed.
            callback(error, null);
        } else {

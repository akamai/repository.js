 ...
 * @param {string | function} source - source of the original call 
 *  (i.e. script or cli)... can also be the callback below 
 * @param {?function} callback - 
 * [Callback called after the request to get the token has completed]{@link Tokens~getToken(token, source, callback)}
 */
Tokens.prototype.getToken = function(token, source, callback) {
    if (typeof source === "function") {
        callback = source;

        api(null, this.endpoint + "/" + token, "GET", null, callback);
    } else {
        api(null, this.endpoint + "/" + token, "GET", null, source, callback);
    }
